...
Another advantage of bar plots is that we can add extra variables to indicate the composition (using another variable) of each of the levels. For example, we will add the variable *trap_type* to color the bars:

```{r}
captures %>% 
  count(municipality, trap_type, wt = treated) %>% 
  ggplot() +
  geom_bar(aes(
...
This can also be done by looking as a proportion by adding the argument `position = 'fill` to the `geom_bar()` function, notice that this removes the sense of number of observations for the main category (*year*):
